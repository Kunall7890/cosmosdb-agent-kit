...
1. [SDK Best Practices](#1-sdk-best-practices) — **HIGH**
   ...
   - 1.13 [Unwrap CosmosItemResponse and enable content response in Java SDK](#113-unwrap-cosmositemresponse-and-enable-content-response-in-java-sdk)
   - 1.14 [Use dependent @Bean methods for Cosmos DB initialization in Spring Boot](#114-use-dependent-bean-methods-for-cosmos-db-initialization-in-spring-boot)
   - 1.15 [Spring Boot and Java version compatibility for Cosmos DB SDK](#115-spring-boot-and-java-version-compatibility-for-cosmos-db-sdk)
   - 1.16 [Initialize Async Cosmos DB Container Before CosmosDBSaver](#116-initialize-async-cosmos-db-container-before-cosmosdbsaver)
   - 1.17 [Use CosmosDBSaver for LangGraph Checkpointing](#117-use-cosmosdbsaver-for-langgraph-checkpointing)
   - 1.18 [Use AzureCosmosDBNoSQLChatMessageHistory for Persistent Conversations in JS/TS](#118-use-azurecosmosdbnosqlchatmessagehistory-for-persistent-conversations-in-js-ts)
   - 1.19 [Configure Azure OpenAI Embedding Deployment Name for JS/TS LangChain](#119-configure-azure-openai-embedding-deployment-name-for-js-ts-langchain)
   - 1.20 [Prevent Filter Injection in JS/TS LangChain Vector Store Queries](#120-prevent-filter-injection-in-js-ts-langchain-vector-store-queries)
   - 1.21 [Configure Full-Text Prerequisites Before JS/TS LangChain Hybrid Search](#121-configure-full-text-prerequisites-before-js-ts-langchain-hybrid-search)
   - 1.22 [Use Managed Identity for JS/TS LangChain Cosmos DB Integration](#122-use-managed-identity-for-js-ts-langchain-cosmos-db-integration)
   - 1.23 [Choose the Correct Search Type for JS/TS LangChain Vector Store](#123-choose-the-correct-search-type-for-js-ts-langchain-vector-store)
   - 1.24 [Use AzureCosmosDBNoSQLSemanticCache for LLM Cost Reduction in JS/TS](#124-use-azurecosmosdbnosqlsemanticcache-for-llm-cost-reduction-in-js-ts)
   - 1.25 [Correctly Initialize AzureCosmosDBNoSQLVectorStore in JavaScript/TypeScript](#125-correctly-initialize-azurecosmosdbnosqlvectorstore-in-javascript-typescript)
   - 1.26 [Use Persistent MCP Client Sessions for Multi-Agent Applications](#126-use-persistent-mcp-client-sessions-for-multi-agent-applications)
   - 1.27 [Handle MCP ToolMessage Content Format Variations](#127-handle-mcp-toolmessage-content-format-variations)
   - 1.28 [Filter MCP Tools by Name Prefix for Agent Assignment](#128-filter-mcp-tools-by-name-prefix-for-agent-assignment)
   - 1.29 [Configure local development environment to avoid cloud connection conflicts](#129-configure-local-development-environment-to-avoid-cloud-connection-conflicts)
   - 1.30 [Explicitly reference Newtonsoft.Json package](#130-explicitly-reference-newtonsoft-json-package)
   - 1.31 [Use the Patch API for atomic counter increments](#131-use-the-patch-api-for-atomic-counter-increments)
   - 1.32 [Configure Preferred Regions for Availability](#132-configure-preferred-regions-for-availability)
   - 1.33 [Include aiohttp When Using Python Async SDK](#133-include-aiohttp-when-using-python-async-sdk)
   - 1.34 [Never share a single CosmosItemRequestOptions instance across multiple createItem calls](#134-never-share-a-single-cosmositemrequestoptions-instance-across-multiple-createitem-calls)
   - 1.35 [Handle 429 Errors with Retry-After](#135-handle-429-errors-with-retry-after)
   - 1.36 [Use consistent enum serialization between Cosmos SDK and application layer](#136-use-consistent-enum-serialization-between-cosmos-sdk-and-application-layer)
   - 1.37 [Reuse CosmosClient as Singleton](#137-reuse-cosmosclient-as-singleton)
   - 1.38 [Annotate entities for Spring Data Cosmos with @Container, @PartitionKey, and String IDs](#138-annotate-entities-for-spring-data-cosmos-with-container-partitionkey-and-string-ids)
   - 1.39 [Use CosmosRepository correctly and handle Iterable return types](#139-use-cosmosrepository-correctly-and-handle-iterable-return-types)
...
### 1.13 Unwrap CosmosItemResponse and enable content response in Java SDK
...
### 1.14 Use dependent @Bean methods for Cosmos DB initialization in Spring Boot
...
### 1.15 Spring Boot and Java version compatibility for Cosmos DB SDK
...
### 1.16 Initialize Async Cosmos DB Container Before CosmosDBSaver
...
### 1.17 Use CosmosDBSaver for LangGraph Checkpointing
...
### 1.18 Use AzureCosmosDBNoSQLChatMessageHistory for Persistent Conversations in JS/TS
...
### 1.19 Configure Azure OpenAI Embedding Deployment Name for JS/TS LangChain
...
### 1.20 Prevent Filter Injection in JS/TS LangChain Vector Store Queries
...
### 1.21 Configure Full-Text Prerequisites Before JS/TS LangChain Hybrid Search
...
### 1.22 Use Managed Identity for JS/TS LangChain Cosmos DB Integration
...
### 1.23 Choose the Correct Search Type for JS/TS LangChain Vector Store
...
### 1.24 Use AzureCosmosDBNoSQLSemanticCache for LLM Cost Reduction in JS/TS
...
### 1.25 Correctly Initialize AzureCosmosDBNoSQLVectorStore in JavaScript/TypeScript
...
### 1.26 Use Persistent MCP Client Sessions for Multi-Agent Applications
...
### 1.27 Handle MCP ToolMessage Content Format Variations
...
### 1.28 Filter MCP Tools by Name Prefix for Agent Assignment
...
### 1.29 Configure local development environment to avoid cloud connection conflicts
...
### 1.30 Explicitly reference Newtonsoft.Json package
...
### 1.31 Use the Patch API for atomic counter increments
...
### 1.32 Configure Preferred Regions for Availability
...
### 1.33 Include aiohttp When Using Python Async SDK
...
### 1.34 Never share a single CosmosItemRequestOptions instance across multiple createItem calls
...
### 1.35 Handle 429 Errors with Retry-After
...
### 1.36 Use consistent enum serialization between Cosmos SDK and application layer
...
### 1.37 Reuse CosmosClient as Singleton
...
### 1.38 Annotate entities for Spring Data Cosmos with @Container, @PartitionKey, and String IDs
...
### 1.39 Use CosmosRepository correctly and handle Iterable return types
